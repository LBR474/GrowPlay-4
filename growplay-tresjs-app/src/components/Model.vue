...
watch(
  ...
  (gltf) => {
    ...
    // ============================================================
    // ADD LETTERS + LOGO TO LETTER GROUP
    // ============================================================

    letters.forEach((letterName) => {
      const letter = gltf.scene.getObjectByName(letterName);
      ...
      if (letter) {
        ...
        console.log("Could not find:", letterName);
      }
    });

    // ============================================================
    // ADD EVERYTHING ELSE TO CLIMBING FRAME GROUP
    // ============================================================
    ...
    climbingFrameGroup.traverse((object) => {
      if ((object as Mesh).isMesh) {
        ...
        if (mesh.name.toLowerCase().includes("foot")) {
    ...
    climbingFrameGroup.traverse((object) => {
      if ((object as Mesh).isMesh) {
        ...
        if (mesh.name.toLowerCase().includes("long_upright")) {
    ...
    climbingFrameGroup.traverse((object) => {
      if ((object as Mesh).isMesh) {
        ...
        if (mesh.name.toLowerCase().includes("piece")) {
    ...
    // ============================================================
    // REVEAL LONG UPRIGHTS SEQUENTIALLY
    // ============================================================
    ...
    // Put them into numerical Blender-name order
    ...
    pieces.forEach((mesh, index) => {
      ...
      materials.forEach((material) => {
        gsap.to(material, {
          ...
          onComplete: () => {
            // ============================================================
            // FINAL FLOURISH — ROTATE CLIMBING FRAME 360°
            // ============================================================

            gsap.to(climbingFrameGroup.rotation, {
              y: climbingFrameGroup.rotation.y + Math.PI * 2,
    ...
    gsap.to(letterGroup.rotation, {
      ...
      y: letterGroup.rotation.y + Math.PI / -2,
    ...
    gsap.to(climbingFrameGroup.scale, {
      y: climbingFrameGroup.scale.y + 0.2,
      x: climbingFrameGroup.scale.x + 0.2,
